...
class Alpha098(Factor):
    """Alpha#98: VWAP成交量相关性与开盘价成交量相关性差因子"""
    id = "alpha_098"
    name = "Alpha#98"
    ...
    def _calculate_factor(self, close, open_price, high, low, vol, pct_chg, vwap, adv20):
        ...
        adv5 = sma(vol, 5)
        adv15 = sma(vol, 15)
        p1 = rank(decay_linear(correlation(vwap, sma(adv5, 26), 5), 7)).iloc[-1]
        p2 = rank(decay_linear(ts_rank(ts_argmin(correlation(rank(open_price), rank(adv15), 21), 9), 7), 8)).iloc[-1]
        return p1 - p2
