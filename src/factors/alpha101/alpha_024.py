...
class Alpha024(Factor):
    ...
    def _calculate_factor(self, close, open_price, high, low, vol, pct_chg, vwap, adv20):
        """Alpha#24: 长期趋势判断因子"""
        cond = delta(sma(close, 100), 100).iloc[-1] / delay(close, 100).iloc[-1] <= 0.05
        if cond:
            return -(close.iloc[-1] - ts_min(close, 100).iloc[-1])
        else:
            return -delta(close, 3).iloc[-1]
