...
class Alpha021(Factor):
    ...
    def _calculate_factor(self, close, open_price, high, low, vol, pct_chg, vwap, adv20):
        """Alpha#21: 均值偏离与成交量组合"""
        cond_1 = sma(close, 8).iloc[-1] + stddev(close, 8).iloc[-1] < sma(close, 2).iloc[-1]
        cond_2 = adv20.iloc[-1] / vol.iloc[-1] < 1
        return -1 if cond_1 or cond_2 else 1
