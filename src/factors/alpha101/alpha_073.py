...
class Alpha073(Factor):
    ...
    required_fields = ['open', 'low', 'close', 'high', 'low']
    min_data_periods = 30
    ...
    def _calculate_factor(self, close, open_price, high, low, vol, pct_chg, vwap, adv20):
        ...
        p1 = rank(decay_linear(delta(vwap, 5), 3)).iloc[-1]
        w = (open_price * 0.147155) + (low * (1 - 0.147155))
        p2 = ts_rank(decay_linear(-1 * delta(w, 2) / w, 3), 17).iloc[-1]
        return -max(p1, p2)
