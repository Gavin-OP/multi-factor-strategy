...
class Alpha022(Factor):
    ...
    def _calculate_factor(self, close, open_price, high, low, vol, pct_chg, vwap, adv20):
        """Alpha#22: 高价成交量相关性变化"""
        df = correlation(high, vol, 5)
        delta_df = delta(df, 5).iloc[-1]
        rank_std = rank(stddev(close, 20)).iloc[-1]
        return -delta_df * rank_std
