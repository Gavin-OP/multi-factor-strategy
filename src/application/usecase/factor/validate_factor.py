...
class ValidateFactorUseCase:
    ...
    def execute(
        ...
        # 获取价格数据
        print(f"[Factor Validation] Fetching price data...")
        price_data = self.tushare_repo.get_daily_multiple(stock_codes, start_date, end_date, limit=50)
        
        # 判断数据来源
        data_source = "tushare" if self.tushare_repo.is_available() else "local"
        ...
        print(f"[Factor Validation] Data: {unique_stocks} stocks, {unique_dates} trading days")
        ...
        quantile_returns = self._calculate_quantile_returns(price_data, factor_values, quantiles)
        
        # 计算 IC 衰减曲线
        print("[Factor Validation] Calculating IC decay...")
        decay_curve = self._calculate_decay_curve(price_data, factor_values, max_lag=min(unique_dates - 1, 10))
        
        # 分析 IC
        ic_stats = self.analyze_service.analyze_ic(ic_series) if ic_series else {
            'ic_mean': 0, 'ic_std': 0, 'icir': 0, 'ic_positive_ratio': 0.5
        }
        
        # 计算单调性
        monotonicity = self.validate_service.calculate_monotonicity(quantile_returns) if quantile_returns else 0.5
        ...
        print(f"[Factor Validation] Done - IC: {ic_stats['ic_mean']:.4f}, Grade: {grade}")
    ...
    def _calculate_decay_curve(
        ...
        for lag in range(1, max_lag + 1):
            ic_series = self._calculate_ic_series(price_data, factor_values, lag)
            if ic_series:
